...
async function updateRoom (roomId: string = '1'): Promise<void> {
  const room: Room = await (await fetch(`/api/game/1/room/${roomId}`)).json()
  roomStore.set(room)
}
...
export async function pickChest (): Promise<void> {
  ...
  const { inventory } = await (await fetch(`/api/game/1/room/${roomId}/pickChest`)).json()
...
export async function craftItem (): Promise<void> {
  const roomId = getRoomId()
  const { inventory } = await (await fetch(`/api/game/1/room/${roomId}/craft`)).json()
  inventoryStore.set(inventory)
}

async function updateInventory (): Promise<void> {
  const { inventory } = await (await fetch('/api/game/1/inventory')).json()
  inventoryStore.set(inventory)
}
